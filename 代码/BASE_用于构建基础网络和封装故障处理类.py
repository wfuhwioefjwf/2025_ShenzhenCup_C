...
# ============ 常量 ============
EXCEL = str((HERE / "C题附件：增加导线中间的开关和是否为干路两列.xlsx").resolve())
V_BASE = 10e3        
...
# 提取干路线路编号（根据"是否是干路"列的值判断）
main_lines = df_topo[df_topo['是否是干路'] == 'True']['编号'].apply(lambda x: f'L{x}').tolist()   
...
def build_network():
    ...
    # 断路器
    circuit_breakers = [{'名称': f'CB{i}', '故障状态': False} for i in range(1, 4)]
    for br in circuit_breakers:
        G.add_node(br['名称'], type='断路器', **br)

    # 分段开关
    segment_switches = [
        {'名称': f'S{i}', '开关状态': "开", '故障状态': False} for i in range(1, 30)
    ]
    for sw in segment_switches:
        G.add_node(sw['名称'], type='分段开关', **sw)
    ...
    # 固定连边（断路器/DG/联络线端点）
    base_connections = [
        ("CB1","U1",False), ("CB3","U23",False), ("CB2","U43",False),
        ("U22","DG2",False), ("U16","DG1",False), ("U39","DG5",False),
        ("U32","DG3",False), ("U35","DG4",False), ("U52","DG7",False),
        ("U55","DG8",False), ("U48","DG6",False),
        ("U13","S13-1",False), ("S13-1","LT_13_43",False), ("LT_13_43","U43",False),
        ("U19","S29-2",False), ("S29-2","LT_19_29",False), ("LT_19_29","U29",False),
        ("U23","S62-3",False), ("S62-3","LT_23_62",False), ("LT_23_62","U62",False),
    ]
    for u, v, is_tie in base_connections:
...
class NetworkOperator:
    ...
    def _configure_initial_state(self):
        dgs = [n for n, d in self.G.nodes(data=True) if d.get('type')=='分布式能源']
        for dg_name in dgs:
            visited = []  # 保存所有路径中的节点
            critical_node=None
            load=0
    ...
    # 添加三故障处理核心函数
    def simulate_three_faults(self, loc1, loc2, loc3):
        # 基础三故障网络
        net0 = copy.deepcopy(self.original_G)
        for loc in (loc1, loc2, loc3):
            if loc not in net0.nodes:
                continue
            net0.nodes[loc]['故障状态'] = True
            if loc.startswith('L'):                 
                net0.remove_node(loc)
            elif loc.startswith(('S', 'CB', 'DG')):       
                net0.nodes[loc]['开关状态'] = '关'

        # 针对每个故障生成候选操作
        cands1 = self._generate_candidates_for_fault(loc1)
        cands2 = self._generate_candidates_for_fault(loc2)
        cands3 = self._generate_candidates_for_fault(loc3)

        # 组合所有候选策略
        combined = []
        for ops1, lbl1 in cands1:
            for ops2, lbl2 in cands2:
                for ops3, lbl3 in cands3:
                    label = f"{lbl1} + {lbl2} + {lbl3}"
                    combined.append((ops1, ops2, ops3, label))
        ...
        f1 = self._determine_fault_type(loc1)
        f2 = self._determine_fault_type(loc2)
        f3 = self._determine_fault_type(loc3)
        ...
        if f2 == 1 or f2 == 5:
            tie_switch_constraints_2 = self._judge_main_fault_type(loc2)
        if f3 == 1 or f3 == 5:
            tie_switch_constraints_3 = self._judge_main_fault_type(loc3)

        # 求三个约束字典的交集
        all_keys = (set(tie_switch_constraints_1.keys()) & 
                   set(tie_switch_constraints_2.keys()) & 
                   set(tie_switch_constraints_3.keys()))
        
        tie_switch_constraints = {}
        for k in all_keys:
            constraint_intersection = (set(tie_switch_constraints_1[k]) & 
                                     set(tie_switch_constraints_2[k]) & 
                                     set(tie_switch_constraints_3[k]))
            if constraint_intersection:  # 只有当交集非空时才添加约束
                tie_switch_constraints[k] = list(constraint_intersection)

        # 组合并评估
        def _merge_ops(*ops_sets):
            d = {}                                 
            for ops in ops_sets:
                for act, sw in ops: d[sw] = act
            return list(d.items())

        # 筛选符合约束的策略组合
        filtered_combined = []
        for ops1, ops2, ops3, label in combined:
            # 合并所有操作
            merged_ops = _merge_ops(ops1, ops2, ops3)
            
            # 提取操作中涉及的开关
            operated_switches = set()
            for act, sw in merged_ops:
                if act == 'open':  # 只关心打开的开关
                    operated_switches.add(sw)
            
            # 检查是否符合约束关系
            valid = True
            for key, constraint_set in tie_switch_constraints.items():
                # 如果key在操作的开关中，则其约束开关至少有一个也在操作中
                if key in operated_switches:
                    if not any(const_sw in operated_switches for const_sw in constraint_set):
                        valid = False
                        break
            
            if valid:
                filtered_combined.append((ops1, ops2, ops3, label))
        ...
        # ============ 评估筛选后的策略 ============
        best = (float('inf'), float('inf'), float('inf'), "无策略")
        for ops1, ops2, ops3, label in filtered_combined:
            net = copy.deepcopy(net0)
            for sw, act in _merge_ops(ops1, ops2, ops3):
                if sw in net.nodes:
                    net.nodes[sw]['开关状态'] = '开' if act == 'open' else '关'
            ...
            # 失负荷
            lost = self._risk(net) + self._extra_user_loss((loc1, loc2, loc3))
            total = (lost + over_penalty)
