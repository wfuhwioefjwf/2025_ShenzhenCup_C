...
class NetworkOperator:
    ...
    # ======== 以下为在原类中添加的带权重灵敏度分析相关函数计算 =========
    # 使用指定的权重计算失负荷
    def _risk_with_weights(self, net, fault_loc=None, weights=None):
        # 如果没有提供权重，使用默认值
        if weights is None:
            weights = TYPE_W
            ...
            if not has_cb:                       
                if gen_cap < load_sum:           
                    ...
                    for u in comp:
                        if u.startswith('U'):
                            user_power = H.nodes[u]['用电功率(kW)']
                            user_weight = weights[H.nodes[u]['类型']]
                            lost += user_power * user_weight

        # 短路用户单独计算
        if fault_loc and fault_loc.startswith('U'):
            nd = net.nodes[fault_loc]
            lost += nd['用电功率(kW)'] * weights[nd['类型']]  # 使用传入的权重

        return lost
    
    # 使用指定的权重计算用户短路额外损失
    def _extra_user_loss_with_weights(self, loc_pair, weights=None):
        if weights is None:
            weights = TYPE_W
        ...
        for loc in loc_pair:
            if loc.startswith('U') and loc in self.original_G.nodes:
                nd = self.original_G.nodes[loc]
                extra += nd['用电功率(kW)'] * weights[nd['类型']]
        return extra
    
    # 使用指定的权重计算单故障风险
    def simulate_fault_with_weights(self, ftype, loc, weights=None, reset=True):
        if weights is None:
            weights = TYPE_W
        ...
        for net, label in candidates:
            ...
            net2 = copy.deepcopy(net)
            lost = self._risk_with_weights(net2, loc, weights)
            total =lost + over_penalty 
            # 结果乘以1e4，保持数据精度
            total = total * 1e4
            lost = lost * 1e4
        ...
        return lost, over_penalty, total, best_strategy, best_overload
    
    # 使用的指定权重计算双故障风险
    def simulate_two_faults_with_weights(self, loc1, loc2, weights=None):
        try:
            if weights is None:
                weights = TYPE_W
                
            # 先构造"基础双故障网络"
            base_net = copy.deepcopy(self.original_G)
            # 应用单个故障到网络
            def _apply_single_fault(g, loc):
                if loc not in g.nodes:
                    return
            ...
            _apply_single_fault(base_net, loc1)
            _apply_single_fault(base_net, loc2)
            ...
            feeder2 = self._find_feeder_root(loc2, self.original_G)
            same_feeder  = (feeder1 == feeder2)
            same_branch  = same_feeder and self._is_same_branch(loc1, loc2, self.original_G)
            ...
            if f1 == f2 == 5:   # 两个都是干路故障，只针对更靠近主干的做控制
                nearer = min((loc1, loc2),
                            key=lambda x: self._distance_to_main(x, self.original_G))
                cs = self._generate_candidates_for_fault(nearer)
                for ops, lbl in cs:
                    combined.append((ops, [], lbl+f"(源:{nearer})"))
            elif not same_feeder or (same_feeder and not same_branch):
            ...
            for ops1, ops2, label in combined:
                try:
                    ...
                    # 失负荷（使用传入的权重）  
                    lost = self._risk_with_weights(net, None, weights) + self._extra_user_loss_with_weights((loc1, loc2), weights)
                    total = (lost + over_penalty)
                    ...
                    print(f"计算{loc1}+{loc2}候选策略时出错: {str(e)}")
                    continue  
            if not best_found:
                print(f"模拟双故障{loc1}+{loc2}整体计算过程出错: 未找到有效策略")
                return 0.0, 0.0, 0.0, "计算失败"
            return best   
        except Exception as e:
            print(f"模拟双故障{loc1}+{loc2}整体计算过程出错: {str(e)}")
            return 0.0, 0.0, 0.0, "计算失败"
...
# 初始化网络（修改init_worker函数以接受权重参数）
def init_worker(base_net, p_cond, prod_safe, custom_weights):
    global TEMPLATE_G, op, P_COND, PROD_SAFE, CUSTOM_WEIGHTS
    TEMPLATE_G = base_net
    op = NetworkOperator(base_net)
    P_COND = p_cond
    PROD_SAFE = prod_safe
    CUSTOM_WEIGHTS = custom_weights  # 存储传入的权重

# 修改 simulate_single 函数，使用带权重的 simulate_fault_with_weights 函数
def simulate_single(loc):
    _reset_graph()

    ftype = op._determine_fault_type(loc)
    lost, over, tot, strat, overload = op.simulate_fault_with_weights(
        ftype, loc, CUSTOM_WEIGHTS, reset=False)

    w = P_COND[loc]
    return loc, lost, over, tot, strat, overload, w

# 修改 simulate_double 函数，使用带权重的 simulate_two_faults_with_weights 函数
def simulate_double(pair):
    loc1, loc2 = pair
    _reset_graph()

    try:
        lost, over, tot, strat = op.simulate_two_faults_with_weights(loc1, loc2, CUSTOM_WEIGHTS)
        if lost is None or over is None or tot is None:
            lost = over = tot = 0.0
            strat = "计算失败"
        w = P_COND[loc1] * P_COND[loc2] / PROD_SAFE
        return loc1, loc2, lost, over, tot, strat, w
    except Exception as e:
        print(f"计算{loc1}+{loc2}故障时出错: {str(e)}")
        return loc1, loc2, 0.0, 0.0, 0.0, f"计算错误: {str(e)[:30]}...", P_COND[loc1] * P_COND[loc2] / PROD_SAFE
...
# 灵敏度分析 - 权重改变对风险评估的影响
def run_sensitivity_analysis(weights_to_vary, weight_values1, weight_values2, base_weights, G_cap, all_faults, p_cond, prod_safe_all):
    global TYPE_W
    
    results_array = np.zeros((len(weight_values1), len(weight_values2)))
    all_sensitivity_results = []
    
    weight1_name, weight2_name = weights_to_vary
    
    for i, w1 in enumerate(weight_values1):
        for j, w2 in enumerate(weight_values2):
            # 更新权重
            current_weights = copy.deepcopy(base_weights)
            current_weights[weight1_name] = w1
            current_weights[weight2_name] = w2
            
            # 赋值全局变量
            TYPE_W = current_weights
            
            # 创建初始网络
            op0 = NetworkOperator(G_cap)
            op0._init_network_status()
            ...
            print(f"\n当前权重: {weight1_name}={w1}, {weight2_name}={w2}")
            ...
            initargs=(op0.G, p_cond, prod_safe_all, current_weights)  # 添加当前权重
        ) as pool:
                
                # 单故障
                cnt = 0
                total_single = len(single_args)
                for loc, lost, over, tot, strat, overload, w in pool.imap_unordered(
                    simulate_single, single_args, chunksize=50):
                    ...
                    if cnt % PROGRESS_INT == 0:
                        print(f"  单故障进度 {cnt}/{total_single}")
                    
                    all_rows.append({
                        "weights": f"{weight1_name}={w1},{weight2_name}={w2}",
                        "type": "single",
                        "loc": loc,
                        "best_strategy": strat,
                        "lost": lost,
                ...
                for loc1, loc2, lost, over, tot, strat, w in pool.imap_unordered(
                    simulate_double, double_args, chunksize=30):
                    ...
                    if cnt % PROGRESS_INT == 0:
                        print(f"  双故障进度 {cnt}/{total_double}")
            ...
            E_loss = sum_loss
            E_over = sum_over
            results_array[i, j] = E_risk
            
            # 添加到总结果列表
            result_item = {"E_risk": E_risk, "E_loss": E_loss, "E_over": E_over}
            result_item.update({k: v for k, v in current_weights.items()})
            all_sensitivity_results.append(result_item)
            
            # 输出当前结果
            print(f"[{weight1_name}={w1}, {weight2_name}={w2}] "
                  f"E(loss)={E_loss:,.2f} E(over)={E_over:,.2f} E(risk)={E_risk:,.2f}")
            
            # 保存到Excel
            file_name = f"灵敏度分析_{weight1_name}{w1}_{weight2_name}{w2}.xlsx"
            output_path = os.path.join(BASE_XLSX_PATH, file_name)
            ...
            with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
                df.to_excel(writer, sheet_name=f"权重分析", index=False)
                print(f"结果已保存至 {output_path}")
    
    # 绘制热图
    plt.figure(figsize=(10, 8))
    sns.heatmap(results_array, annot=True, fmt=".2f", 
                xticklabels=weight_values2, yticklabels=weight_values1,
                cmap="YlOrRd")
    plt.xlabel(f"{weight2_name}权重")
    plt.ylabel(f"{weight1_name}权重")
    plt.title(f"{weight1_name}-{weight2_name}权重变化对期望风险的影响")
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_XLSX_PATH, f"{weight1_name}{weight2_name}权重热图.png"))
    plt.close()
...
if __name__ == '__main__':
    ...
    PROGRESS_INT = 50
    BASE_XLSX_PATH = str((HERE / "灵敏度分析_权重").resolve())
    
    dg_nodes = [n for n, d in G.nodes(data=True) if d.get('type') == '分布式能源']

    G_cap = copy.deepcopy(G)
    for dg in dg_nodes:
        G_cap.nodes[dg]['容量(kW)'] = 300

    # -创建输出目录
    os.makedirs(BASE_XLSX_PATH, exist_ok=True)
    # ---------- 2. 权重灵敏度分析参数 ----------
    base_weights = {'居民':0.067, '商业':0.226, '政府和机构':0.354, '办公和建筑':0.354}
    multipliers = [0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4]
    ...
    sensitivity_params = [
        # 第一组: 居民和商业
        {
            'weights_to_vary': ('居民', '商业'),
            'weight_values1': [round(base_weights['居民'] * m, 3) for m in multipliers],
            'weight_values2': [round(base_weights['商业'] * m, 3) for m in multipliers]
        },
        # 第二组: 政府和机构、办公和建筑
        {
            'weights_to_vary': ('政府和机构', '办公和建筑'),
            'weight_values1': [round(base_weights['政府和机构'] * m, 3) for m in multipliers],
            'weight_values2': [round(base_weights['办公和建筑'] * m, 3) for m in multipliers]
        }
    ]
    ...
    for idx, params in enumerate(sensitivity_params, 1):
        print(f"\n===== 开始第{idx}组灵敏度分析: {params['weights_to_vary'][0]}和{params['weights_to_vary'][1]}权重 =====")
        
        results = run_sensitivity_analysis(
            params['weights_to_vary'], 
            params['weight_values1'], 
            params['weight_values2'],
            base_weights, G_cap, all_faults, p_cond, prod_safe_all
        )
    ...
    sensitivity_df.to_excel(os.path.join(BASE_XLSX_PATH, "灵敏度分析汇总.xlsx"), index=False)
