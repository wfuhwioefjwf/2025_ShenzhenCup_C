...
# ============ 常量 ============
EXCEL = str((HERE.parent / "C题附件：增加导线中间的开关和是否为干路两列.xlsx").resolve()) # 基本参数excel
V_BASE = 10e3        
...
# ============ 新增：全局概率字典 ============
# 失负荷概率字典 - 每个用户对应一个概率值
user_outage_probability = {f'U{i}': 0.0 for i in range(1, 63)}
# 过负荷概率字典 - 每个用户对应一个概率值  
user_overload_probability = {f'U{i}': 0.0 for i in range(1, 63)}
...
def run_distflow(G):
    ...
    try:
        ...
        # 创建line_id到节点对的映射
        line_to_nodes = {}
        for head, tail, data in D.edges(data=True):
            line_id = data.get('id')
            if line_id:
                line_to_nodes[line_id] = (head, tail)
        ...
        for line_id, I in I_amp.items():
            if I > 1.1 * RATED_CURRENT:
                ...
                # 将line_id对应的head和tail加入到over_users
                if line_id in line_to_nodes:
                    head, tail = line_to_nodes[line_id]
...
class NetworkOperator:
    ...
    def simulate_fault(self, ftype, loc, reset=True):
        ...
        if ftype == 1:
            ...
            overload, over_users = run_distflow(new_net)
            if not overload:
        ...
        for net, label in candidates:
            ...
            # 计算潮流
            overload, over_users = run_distflow(net)
            ...
            lost, lost_users = self._risk(net2, loc)
            ...
            if total < best_risk[2] or (total == best_risk[2] and over_penalty < best_risk[1]):
                ...
                best_loss_users = lost_users
                best_over_users = over_users
         
        return (*best_risk, best_strategy, best_overload, best_loss_users, best_over_users)
    ...
    def simulate_two_faults(self, loc1, loc2):
        ...
        best = (float('inf'), float('inf'), float('inf'), "无策略", set(), set())
        for ops1, ops2, label in combined:
            ...
            # 失负荷  
            lost_1 , lost_users_1 = self._risk(net)
            lost_2 , lost_users_2 = self._extra_user_loss((loc2, loc1))

            lost = lost_1 + lost_2
            lost_users = lost_users_1.union(lost_users_2)
            ...
            if total < best[2]:
                best = (lost, over_penalty, total, label, lost_users, over_users)
...
def simulate_single(loc):
    ...
    lost, over, tot, strat, overload, lost_users, over_users = op.simulate_fault(
        ftype, loc, reset=False)
    w = P_COND[loc]      # 计算概率         
    
    # 返回用户影响信息
    return loc, lost, over, tot, strat, overload, w, lost_users, over_users
...
def simulate_double(pair):
    ...
    lost, over, tot, strat, lost_users, over_users = op.simulate_two_faults(loc1, loc2)
    w = P_COND[loc1] * P_COND[loc2] / PROD_SAFE     # 计算概率  
    
    # 返回用户影响信息
    return loc1, loc2, lost, over, tot, strat, w, lost_users, over_users
...
if __name__ == '__main__':
    CAP_SEQ       = [i for i in range(300, 301, 30)]
    PROGRESS_INT  = 100
    ...
    for idx, cap in enumerate(CAP_SEQ, 1):
        ...
        with mp.Pool(processes=cpu_num,
            ...
            for result in pool.imap_unordered(simulate_single, single_args, chunksize=50):
                loc, lost, over, tot, strat, overload, w, lost_users, over_users = result
                cnt += 1
                sum_loss += lost * w
                sum_over += over * w
                sum_risk += tot * w
                ...
                for user in lost_users:
                    if user in user_outage_probability:
                        user_outage_probability[user] += w
                
                # 更新过负荷概率字典
                for user in over_users:
                    if user in user_overload_probability:
                        user_overload_probability[user] += w
            ...
            for result in pool.imap_unordered(simulate_double, double_args, chunksize=30):
                loc1, loc2, lost, over, tot, strat, w, outage_users, overload_users = result
                cnt += 1
                sum_loss += lost * w
                sum_over += over * w
                sum_risk += tot * w
        ...
        print("失负荷概率字典:")
        print("="*60)
        for user, prob in sorted(user_outage_probability.items()):
            if prob > 0:  # 只打印概率大于0的用户
                print(f"{user}: {prob:.6f}")
        
        print("\n" + "="*60)
        print("过负荷概率字典:")
        print("="*60)
        for user, prob in sorted(user_overload_probability.items()):
            if prob > 0:  # 只打印概率大于0的用户
                print(f"{user}: {prob:.6f}")
        
        # ============ 新增：将失负荷和过负荷概率表写入Excel ============
        # 创建概率统计Excel文件
        BASE_XLSX_DIR = str((HERE).resolve())
        prob_filename = f"失负荷和过负荷概率_{int(cap)}kW.xlsx"

        prob_path = os.path.join(BASE_XLSX_DIR, prob_filename)
        
        # 创建概率统计工作簿
        prob_workbook = openpyxl.Workbook()
        
        # 创建失负荷概率表
        outage_sheet = prob_workbook.active
        outage_sheet.title = "失负荷概率"
        outage_headers = ["用户编号", "失负荷概率"]
        outage_sheet.append(outage_headers)
        
        # 写入失负荷概率数据
        for user, prob in sorted(user_outage_probability.items()):
            outage_sheet.append([user, prob])
        
        # 创建过负荷概率表
        overload_sheet = prob_workbook.create_sheet(title="过负荷概率")
        overload_headers = ["用户编号", "过负荷概率"]
        overload_sheet.append(overload_headers)
        
        # 写入过负荷概率数据
        for user, prob in sorted(user_overload_probability.items()):
            overload_sheet.append([user, prob])
        
        # 创建汇总表
        summary_sheet = prob_workbook.create_sheet(title="概率汇总")
        summary_headers = ["用户编号", "失负荷概率", "过负荷概率", "总风险概率"]
        summary_sheet.append(summary_headers)
        
        # 写入汇总数据
        for user in sorted(user_outage_probability.keys()):
            outage_prob = user_outage_probability[user]
            overload_prob = user_overload_probability[user]
            total_risk_prob = outage_prob + overload_prob  # 总风险概率
            summary_sheet.append([user, outage_prob, overload_prob, total_risk_prob])
        
        # 保存概率统计Excel文件
        prob_workbook.save(prob_path)
        prob_workbook.close()
        
        print(f"\n概率统计表已保存至: {prob_filename}")
        print(f"包含以下工作表:")
        print(f"  - 失负荷概率: 各用户的失负荷概率")
        print(f"  - 过负荷概率: 各用户的过负荷概率") 
        print(f"  - 概率汇总: 失负荷、过负荷及总风险概率汇总")
